...
export default function LandingPage() {
  ...
  const features = [
    ...
    { title: 'Friends & leaderboards', desc: 'Compete daily, weekly, or all-time. Share sessions, give kudos.', species: 'cat', color: '#F5C518', color2: '#D9A400' },
  ...
  return (
    <div className="page-fade" style={{ maxWidth: 680, margin: '0 auto', padding: '80px 20px' }}>
      <div style={{ textAlign: 'center', marginBottom: 60 }}>
        <h1 className="gradient-text" style={{ fontSize: 52, marginBottom: 16, lineHeight: 1.1 }}>
          Anchor
        </h1>
        <p style={{ fontSize: 18, color: 'var(--text-muted)', maxWidth: 440, margin: '0 auto 32px' }}>
          Stay locked in. Turn focused study time into XP, streaks, and friendly competition — with a planner that actually keeps up with your week.
        </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 16 }}>
        {features.map((f) => (
          <div key={f.title} className="glass-card" style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
